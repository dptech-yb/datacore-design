...
import type { ComponentDoc, ExampleKind } from "../catalog/types";
import { categoryOf, getComponent } from "../catalog";
import { navigate } from "../router";
import { ExamplePlayer } from "../components/ExamplePlayer";

const EXAMPLE_GROUPS: { kind: ExampleKind; heading: string; anchor: string }[] = [
...
export function componentToc(doc: ComponentDoc) {
  const toc = EXAMPLE_GROUPS.filter((group) => doc.examples.some((example) => example.kind === group.kind))
    ...
    <article className="doc-page">
      <header className="doc-header">
        ...
        <div className="doc-badges">
          ...
          <a className="version-chip chip-link" href={`./llms-full.txt#dc${doc.id.replace(/-/g, "")}`} target="_blank" rel="noreferrer">
